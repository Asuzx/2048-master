...
import tk.woppo.mgame.http.MyHttp;
...
public class MainView extends View implements View.OnClickListener {
    ...
    private void initDialog() {
        if (builder == null) {
            ...
            View view = LayoutInflater.from(activity).inflate(R.layout.activity_rank, null);
            ...
            tv_score.setText("本次得分" + game.score);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_enter:
                if (!"".equals(ed_user.getText().toString())) {
    ...
    private void http() {
        ...
        try {
            json.put("user",ed_user.getText().toString());
            json.put("score",game.score);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        RequestBody requestbody = RequestBody.create(JSON, String.valueOf(json));
        Request request = new Request.Builder()
                .url(activity.getResources().getString(R.string.upload))
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    Toast.makeText(activity, "上传失败请检测网络", Toast.LENGTH_SHORT).show();
                    break;
                case 1:
    ...
    private void createEndGameStates(Canvas canvas, boolean win, boolean showButton) {
        ...
        if (win) {
            ...
            paint.setTextSize(gameOverTextSize);
            paint.setTextAlign(Paint.Align.CENTER);
            canvas.drawText(getResources().getString(R.string.high_score), middleX, middleY - centerText(), paint);
    ...
    private void createBackgroundBitmap(int width, int height) {
        ...
        drawRankButton(canvas,false);
    }
    private void drawRankButton(Canvas canvas,boolean lightUp) {
        drawDrawable(canvas,
                backgroundRectangle,
                sRank,
                sYIcons, sRank + iconSize,
                sYIcons + iconSize
        );

        drawDrawable(canvas,
                getResources().getDrawable(R.drawable.ic_launcher),
                sRank + iconPaddingSize,
                sYIcons + iconPaddingSize,
                sRank + iconSize - iconPaddingSize,
                sYIcons + iconSize - iconPaddingSize
        );
    ...
    private void getLayout(int width, int height) {
        ...
        sRank = sXUndo- iconSize * 3 / 2 - iconPaddingSize;
        resyncTime();
    }
    ...
    public MainView(Context context, Context mcontext) {
        super(context);
